...
def task8():
    group_id = request.args.get('group_id',type=int)
    mountain_id = request.args.get('mountain_id',type=int)
    start = request.args.get('start')
    end = request.args.get('end')
    cursor =  mysql.connection.cursor(named_tuple=True)
    cursor.execute('INSERT INTO `Climbing`(`group_id`, `mountain_id`, `start`, `end`) VALUES (%s, %s, %s, %s)', (group_id, mountain_id, start,end,))
    mysql.connection.commit()
    cursor.close()
    return jsonify('ok')